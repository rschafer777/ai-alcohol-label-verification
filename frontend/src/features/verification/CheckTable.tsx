...
function CorrectionEditor({ check, onCancel, onSave }: { check: CheckResult; onCancel: () => void; onSave: (value: string) => void }): ReactElement {
  const [value, setValue] = useState(check.observedDisplay ?? "");
  return (
    <form aria-label={`Correct the read value for ${displayLabel(check)}`} onSubmit={(event) => { event.preventDefault(); if (value.trim()) onSave(value.trim()); }} style={{ display: "grid", gap: 6 }}>
      <input aria-label="Corrected value" autoFocus className="input" maxLength={240} onChange={(event) => setValue(event.target.value)} value={value} />
      <span style={{ display: "flex", gap: 6 }}>
        <button className="btn btn-primary" style={{ fontSize: 12, padding: "4px 10px" }} type="submit">Re-check</button>
                    ...
                    {rows.map((check) => {
                      ...
                      return (
                        <tr aria-selected={selected} className={check.applicable ? "" : "na"} key={check.checkId}>
                          <td className="check-name">{displayLabel(check)}{correctedIds.has(check.checkId) ? <><br /><span className="tag tag-outline" style={{ fontSize: 10 }}>Reviewer-corrected</span></> : null}</td>
                          ...
                          <td className="small">
                            {editing === check.checkId && onCorrect ? <CorrectionEditor check={check} onCancel={() => setEditing(null)} onSave={(value) => { setEditing(null); onCorrect(check, value); }} /> : (
                              <span className="read-cell"><span>{observedDisplay(check, result)}</span>{editable ? <button aria-label={`Correct the read value for ${displayLabel(check)}`} className="btn btn-ghost btn-icon" onClick={() => setEditing(check.checkId)} type="button">{icons.pencil(14)}</button> : null}</span>
                            )}
                          </td>
                          ...
                          <td className="right">{check.evidenceRef ? <button aria-pressed={selected} className="btn btn-ghost show-btn" onClick={() => onSelect(check.checkId)} type="button">{icons.target(14)} Show</button> : null}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {rows.map((check) => {
                ...
                return (
                  <article aria-selected={selected} className={`card check-card${check.applicable ? "" : " na"}`} key={check.checkId}>
                    <div className="card-head"><span className="card-title">{displayLabel(check)}{correctedIds.has(check.checkId) ? <> <span className="tag tag-outline" style={{ fontSize: 10 }}>Reviewer-corrected</span></> : null}</span><Badge applicable={check.applicable} state={check.state} /></div>
                    <dl>
