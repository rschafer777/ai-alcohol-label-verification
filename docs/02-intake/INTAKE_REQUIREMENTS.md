...
| 1.4 | 2026-09-05 | Classified production-scale representative Azure load qualification separately from time-boxed demo-UAT entry | Product-owner release direction |
...
| INT-Q-011 | Production-scale deployed measurement | Before any production-scale Azure performance or p95 claim, use sanitized representative products with distinct admitted pixel hashes and disclosed beverage, panel, difficulty, and dimension distributions; cold startup is separate from post-ready latency, rate and queue waits remain in total batch time, and repeated or re-encoded samples remain diagnostic evidence only. This operational qualification does not block time-boxed demo requester UAT when the exact-build deployment, local representative corpus, local capacity gates, and distinct Azure beverage-family smoke pass |
