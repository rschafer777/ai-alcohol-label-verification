...
**Status:** Corrected OCR implementation deployed and live proof passed  
...
Run `33577226574` passed every protected source gate, the exact-archive privacy scan, all 30 governed product cases, OIDC authentication, immutable OCI build and push, local digest readiness, Azure effective-configuration readback, HTTPS controls, and one complete public sample verification. It deployed commit `4024aded5beae416d471f13cbdb5563572a06328` with image digest `sha256:5521af5a3714cfb78f5329f91213ae1ef1a79c392a502b16c94697e345803715`. Independent browser UAT then reproduced a real 504 worker timeout on the two-panel built-in sample, including a retry. A direct public API attempt reached the same 6.25-second boundary. The original 1 vCPU profile forced two OCR lanes, each configured for two ONNX Runtime threads, to compete for one vCPU. At that point, the correction assigned 2 vCPU and 4 GiB to the single scale-to-zero replica, kept concurrency at one, and strengthened the deployment gate from one sample attempt to three consecutive attempts with a mean below 5 seconds and no attempt at or above 9 seconds. The later successful run recorded below closed this issue.

Run `33578923408` passed all source, privacy, product, OIDC, image, local readiness, Azure deployment, and effective-configuration gates for commit `fd9a030e06bb699a51507bc4ecdb2d0c6e926caa`. The strengthened three-run public gate rejected the revision, and rollback restored the prior governed digest successfully. The effective 2 vCPU resource matched the template, but the adapter still assigned two intra-operation threads to each of two simultaneous OCR lanes and warmed only the first lane. The final correction assigns one ONNX Runtime intra-operation thread to each lane and warms both engines before readiness. The timing summary is emitted before the performance assertion so every successful HTTP result remains visible even when its statistical gate fails.

Run `33583826159` passed the complete protected workflow for commit `ee0a15160643c42d1d2b92646442927e53d7f21b` and image digest `sha256:c895a059a234255042c51de4030f6fe65c686d6f0fdcb439b2ff9226d8947363`. Source gates, the exact public-archive privacy scan, all 30 product cases, OIDC, immutable image construction and push, local-digest readiness, Azure deployment, effective-configuration readback, HTTPS controls, metadata, and the strengthened public result gate all passed. The three workflow-controlled server durations were 5,027.237, 4,543.133, and 4,573.130 ms, with a 4,714.500 ms mean and 5,027.237 ms maximum. Rollback was skipped because every deployment assertion passed.

Independent live UAT then loaded the built-in sample through the public browser, rendered all 19 checks, displayed 4.4 seconds of server work, and returned Review needed with only warning wording at Review and warning physical size at Not verified. Three additional direct public API attempts completed at 4,361.087, 4,387.021, and 4,363.800 ms with the same exact result shape and build ID. Machine-readable evidence is retained in `docs/08-validation/evidence/azure-live-uat.json`.
