...
| VP-13 Image oracle and pixel ground truth | Compare every current image with the disposition oracle (`test-oracle-v1.json`, 42 matching files) and with the field-level pixel ground truth (`pixel-ground-truth-v1.json`, all 70 files) using `scripts/score_ground_truth.py` | Field accuracy, warning-wording outcomes, false-clean and false-reject counts, and timing are reported per run; the ground truth is evaluation-only and the runtime never reads it; a TTB reviewer determination remains a requester UAT item |
