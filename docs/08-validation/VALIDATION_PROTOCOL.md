...
| VP-25 Representative deployed performance | Run sanitized one-panel through three-panel products with distinct admitted pixel hashes and a disclosed distribution on the governed Azure revision; measure cold startup, post-ready requests, and 20-product and 300-product paths separately | Post-ready mean no more than 5 seconds, at least 75 percent of normal products within 5 seconds, every selected difficult product below 9 seconds, 20-product wall time no more than 100 seconds, 300-product wall time no more than 1,500 seconds, no restart, timeout, drop, or memory failure; at least 100 distinct product fingerprints for formal p95 |
...
- The 30-distinct-product run is a quick deployed gate. At least 100 distinct product request fingerprints are required to report a formal deployed p95. A governed cache-disabled mode can support diagnostics but cannot replace distinct products for the formal claim.
