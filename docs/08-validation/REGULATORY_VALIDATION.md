...
| Exact text: "GOVERNMENT WARNING: (1) According to the Surgeon General, women should not drink alcoholic beverages during pregnancy because of the risk of birth defects. (2) Consumption of alcoholic beverages impairs your ability to drive a car or operate machinery, and may cause health problems." | 27 CFR 16.21 | `warning_wording`: the read is compared word for word with the statute. An exact read is a match; letter case and spacing are not differences because 16.22 fixes the case of the heading only and OCR invents spacing around the clause markers; a clause number fused to the next word ("1According") is an unresolved marker bracket, an OCR signature no label prints. Any other punctuation difference is a review item that names the marks in question, never cleared by the machine, because a photograph cannot settle it. A missing sentence, a statutory word dropped from the middle of a cleanly read statement ("women should drink"), replacement language, or clear word substitutions that outnumber the OCR slips in the read are differences; a word fragment that is the start or end of the statutory word ("ccording", "eral") is a word cut by the edge of the photograph, six or more missing words mark a fragment, and a read peppered with slips, or truncated at either end, goes to review. When a product is submitted as several images, the statement is read on each image that carries it, the best-read image decides (a complete clean read outranks a partial or noisy one, since one printed statement cannot differ between photographs), and when no single image shows the whole statement, a statutory word read in its place on any image is confirmed for the product; that confirmation is a review item because punctuation cannot be settled from the pieces | Implemented, `warnings.py`, `normalize.py` |
| "GOVERNMENT WARNING" in capital letters | 27 CFR 16.22(a)(2) | `warning_heading_uppercase`: exact uppercase heading is a match, title or mixed case is a difference. An uppercase heading cut off at the edge of the photograph ("GOVERNMENT WARNIN") is a review item, and a line that begins mid-heading but continues with the statutory opening ("...ERNMENT WARNING: (1) ACCORDING") anchors a body fragment with no heading: it feeds the cross-image read but never leads the checks, and on its own it can only ask for review or another photograph. A "Proposition 65 warning" or other cautionary note never passes as the federal heading because the statutory opening must follow | Implemented |
