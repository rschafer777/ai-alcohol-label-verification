...
Post-deployment distinct-hash smoke checks submitted three products comprising four difficult photographs to the same public API. The malt result safely read the partial brand `REVOLVER` plus `TEXAS STYLE ALE`; the wine result read `CASCADE LIGHT`, `RIESLING`, 11.5 percent ABV, and 750 mL; the spirits result read `JACK DANIEL'S`, the safe class `WHISKEY`, 40 percent ABV, 80 proof, and 375 mL. All selected the correct beverage family, returned 24 checks, and conservatively routed identified missing, OCR-uncertain, or presentation-uncertain checks to Review. Server durations were 6,176.807, 7,106.857, and 6,917.043 milliseconds, for a 6,733.569-millisecond mean and 7,106.857-millisecond maximum. This small difficult-image smoke passes the 9-second hard-case ceiling; it is not a representative deployed-performance or workload-mean claim. Request IDs, returned build IDs, panel counts, image hashes, dimensions, blocking check IDs, and cause categories are recorded in `../10-release/DEPLOYMENT_EVIDENCE.json`.
