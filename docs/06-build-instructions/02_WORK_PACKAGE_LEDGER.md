...
| `WP-009` | `E-003/S-009` Package and prove runtime characteristics | L | `INT-LEAD`, supported by `ENG-BE` | `WP-004`, `WP-006`, `WP-008` | `TASK-053` perform a clean multi-stage non-root OCI build and clean rebuild; `TASK-054` record image digest, runtime identity, governed-asset hashes, non-root state, and readiness smoke; `TASK-055` optimize cold readiness; `TASK-056` run local warm/cold/load/memory tests; `TASK-057` implement the Azure Container Apps template and OIDC deployment workflow; `TASK-058` read back and test every deployment-controlled assertion without promoting an unexecuted assertion to PASS | `FR-031`; supports `FR-011`, `FR-028`, `FR-040` runtime evidence | Successful hash-recorded OCI build and clean rebuild, non-root/readiness smoke, local performance report, governed Azure configuration, and exact external checklist. If no OCI builder or authorized deployment is available, the affected assertions remain BLOCKED or PENDING and cannot pass. |
