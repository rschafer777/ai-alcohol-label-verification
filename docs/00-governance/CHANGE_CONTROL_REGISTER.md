...
| Exit gate | Corrective requirements, implementation, automated gates, an evidence-reviewed sealed-holdout utility result, zero new false clean, representative distinct-hash local tests, exact-build Azure workflow and three-family difficult-image smoke, documentation, release manifest, and three final independent reviews pass. Production-scale distinct-product Azure load qualification is required before a production performance or p95 claim, not before demo requester UAT |
...
| 2026-09-05 | Classify production-scale Azure load qualification | Time-boxed demo requires exact-build deployment and representative local evidence; limited distinct Azure smoke verifies live family paths without claiming production-scale performance | Approved for demo requester UAT; pre-production qualification remains open |
