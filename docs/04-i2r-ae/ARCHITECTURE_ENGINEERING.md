...
| 1.4 | 2026-09-05 | Separated demo-UAT deployment smoke from pre-production production-scale performance qualification | Product-owner release direction |
...
Demo-UAT entry binds the full local representative corpus and capacity results to the exact governed 4-vCPU and 8-GiB Azure revision, then exercises distinct difficult malt, wine, and spirits inputs as live smoke. Formal production-scale deployed timing uses sanitized representative products with distinct admitted pixel hashes and publishes beverage-family, one-to-three-panel, normal-versus-difficult, and image-dimension distributions. Thirty distinct products provide a quick operational gate. At least 100 distinct product request fingerprints are required for a formal deployed p95 claim. Re-encoding, metadata changes, cache-busted duplicates, and repeated governed samples are diagnostic smoke evidence only. A governed cache-disabled mode may replace uniqueness for a diagnostic comparison but cannot establish the representative p95 claim.
