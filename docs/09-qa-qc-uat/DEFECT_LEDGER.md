...
| `VAL-004` | High | Cold-start resource envelope | Concurrent OCR-engine construction exceeded the original 2 GiB cold-start target. | Both inference engines allocated startup working memory at the same time and previously used more ONNX threads than the selected Azure CPU profile. | Construct the two engines sequentially, assign one ONNX intra-operation thread per lane, warm both lanes concurrently before readiness, close multiprocessing queues on supervisor teardown, and distinguish the steady-state operating target from the selected 4 GiB cold-start envelope. | Warm peak is 1,466,265,600 bytes and 300-row batch peak is 847,306,752 bytes inside the 2 GiB operating target; cold peak is 2,997,751,808 bytes inside the 4 GiB Azure runtime envelope; warm p95 is 2,996.256 ms and readiness through first result is 9,659.653 ms. | CLOSED |
...
| `RTV2-003` | High | Lifecycle and total-phase evidence retained source hashes from before the final test-source changes. | Rerun both governed evidence producers on the corrected snapshot and rebind the assertion ledger. | At the V2 correction gate, lifecycle evidence passed 46 focused and 182 full tests, while total-phase evidence passed 16 backend and 4 frontend tests. Later current-source reruns supersede those historical counts and are recorded under `QA-003` and the active validation record. | CLOSED |
...
| ID | Severity | Description | Corrective action | Regression evidence | Status |
|---|---|---|---|---|---|
| `PUB-001` | High | Active release records retained superseded test, performance, and 50-image counts after the final correction wave. | At that release-integrity gate, rebind both active release records to the then-current 182 Python, 46 frontend, performance, memory, batch, and 0 pass, 45 review, 5 difference evidence. | Three independent RTs verified those then-current claims. Later active records and evidence supersede the historical counts. | CLOSED |
