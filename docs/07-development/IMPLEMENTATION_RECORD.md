...
## Accuracy revision of 2026-09-03

The revision on branch `accuracy-verification-2026-09` changed how evidence is read and how much the machine is allowed to conclude from it. OCR runs once per panel on a working image of at most 2.07 megapixels, then re-reads the government warning and any missing field from an enlarged crop of the region it located; at most three focused crops per request, warning crops first. Boxes from several views are fused by region only when their line heights agree and their texts nearly agree, so a small line inside a tall box and two distinct lines that happen to overlap keep their own places, while a duplicate detection of one line within a single view is fused. Every line keeps stroke width, letter height (the upper quartile of connected-component heights), gray-level range, and a WCAG luminance contrast ratio measured on the view that read it.

Candidate extraction gained glued-word repair for class words ("OrganicVodka", "GRAPEWINE"), tolerance for OCR-damaged units and role words ("12 FL. 0Z", "Alc./Vo", "brewed and bottld by"), a country vocabulary that every origin phrase must satisfy, nested-reading collapse for same-family class designations, address exclusion for appellations, and brand prominence scoring that excludes dated establishment copy and lines that are mostly digits. Beverage type falls back to producer and proof hints when no class word is read.
...
An evaluation harness (`scripts/score_ground_truth.py`) scores every private test image against a pixel-level field ground truth and the disposition oracle; the runtime never reads either file, and no product name, filename, or expected value appears in runtime code.
